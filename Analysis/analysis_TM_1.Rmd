...
```{r}
# dbWriteTable(conn = con,name =  "wos", value = data.frame(wos))
# dbWriteTable(conn = con,name =  "scopus", value = data.frame(scopus))
# dbWriteTable(con, "scopus_wos_final", scopus_wos_final)
# dbWriteTable(con, "edgelist", asn_TM_connected |> 
#                activate(edges) |> as_tibble())
# dbListTables(con)
```

```{r}
dbListFields(con, "scopus_wos_final")
```
...
## WoS Journal Citation Network

```{r}
# wos_ref_journal <-
#   wos_scopus_tos$df  |> 
#   filter(ref_type %in% "wos") |> 
#   select(JI_main = JI, CR) |> slice(1)
#   mutate(JI_main = str_remove_all(JI_main, "\\.")) |> 
#   separate_rows(CR, sep = ";") |> 
#   mutate(JI_ref = str_remove(CR, ".*[0-9]{4}, ")) |> 
#   mutate(JI_ref = str_remove(JI_ref, ",.*")) |> 
#   filter(!str_detect(JI_ref, "[0-9]")) |> 
#   select(-CR) |> 
#   filter(!is.na(JI_ref))
```
...
```{r}
# scopus_ref_journal <- # We have all the journals from main source
#   wos_scopus_tos$df |> 
#   filter(ref_type %in% "scopus") |> 
#   select(CR, SO, JI) |> 
#   separate_rows(CR, sep = ";") |> 
#   mutate(SO = str_remove(CR,
#                          ".*\\([0-9]{4}\\)")) |> 
#   mutate(SO = str_remove(string = SO, ",.*")) |> 
#   mutate(SO =str_trim(string = SO, side = "both")) |> View()
# filter(SO != "") |> 
...
#   scopus_ref_journal |> 
#   left_join(journals_all_abbr,by = c("SO" = "journal_abbr"))
# 
...
## Scopus + WoS Journal Citation Network

Merging both datasets WoS and Scopus, and creating the graph
...
# Saving resutls in database 

# dbWriteTable(con, 
#              "journal_citation_network_nodes", 
#              journal_citation_graph_weighted_tbl |> 
#                activate(nodes) |> 
#                data.frame())
# 
# dbWriteTable(con, 
#              "journal_citation_network_edges", 
#              journal_citation_graph_weighted_tbl |> 
#                activate(edges) |> 
#                data.frame())


# Filtering the network to make it more readable in a figure
...
```{r}
nodes <- 
  journal_citation_graph_weighted_tbl_small_fig_gephi |> 
  activate(nodes) |> 
  as_tibble() |> 
  rename(author = name) |> 
...
graph_from_data_frame(d = edges, vertices = nodes, directed = FALSE) |> 
  write.graph(here("output",
                   "journal_citation_graph.graphml"),
              "graphml")
...
asn_TM_selected_nodes <- 
  asn_TM_connected |> 
  activate(nodes) |> 
  filter(name %in% nodes_selected_10$name) |> 
...
asn_igraph |> write_graph("output/asn_igraph.graphml", 
                          "graphml")
```

Exporting selected graph
...
asn_igraph_subset <- 
  graph_from_data_frame(d = asn_edges_subset, 
                        directed = FALSE, 
                        vertices = asn_nodes_subset)

asn_igraph_subset |> 
  write_graph("output/asn_igraph_subset.graphml", 
              "graphml")
```
...
Cluster 1 - Li, Ximing egonetwork analysis

```{r}
li_ximing_scopus <- 
  bibliometrix::convert2df(file = here("data",
                                       "Li Ximing.bib"),
                           dbsource = "scopus", 
                           format = "bibtex"
  )
li_ximing_ego_network <- 
  li_ximing_scopus |> 
  select(SR, AU) |> 
  separate_rows(AU, sep = ";") |> 
  group_by(SR) |> 
  ...
  filter(from != to) |> 
  mutate(PY = str_extract(SR, pattern = "[0-9]{4}"))
```
...
```{r}
li_yufeng_scopus <- 
  convert2df(file = here("data", "Li Yuefeng.bib"), 
             dbsource = "scopus", 
             format = "bibtex") 
```
...
Counting Li Yufeng friends

```{r}
li_yufeng_friends <- 
  li_yufeng_egonetwork |>
  select(friend = from) |> 
  bind_rows(li_yufeng_egonetwork |> 
              select(friend = to)) |> 
  filter(friend != "LI Y") |> 
  count(friend, sort = TRUE)
```
...
```{r}
yang_liu_scopus <- 
  convert2df(file = here("data", "Yang Liu.bib"), 
             dbsource = "scopus", 
             format = "bibtex") 
```
...
```{r}
sungyoung_lee_scopus <- 
  convert2df(file = here("data", "Sungyoung Lee.bib"), 
             dbsource = "scopus", 
             format = "bibtex") 
```
...
sungyoung_lee_egonetwork <- 
  ...
  expand(from = AU, to = AU) |> 
  filter(from != to) |>
  left_join(sungyoung_lee_scopus |> 
              select(SR, TI), 
            by = "SR")
ungroup() |> 
  select(-SR) |> 
...
```{r}
tm_graph_tbl <- 
  read_graph(file = here("output", "citation_network_full TM.graphml"), 
             format = "graphml") |> 
  as_tbl_graph()
```
...
# figure_5b <- 
  tm_graph_tbl |> 
  activate(nodes) |> 
  mutate(year = str_extract(string = full_name, pattern = ".*([0-9]{4})")) |> 
  data.frame() |> 
  select(year) |> slice(1)
...
```{r}
write_csv(wos_scopus_tos$df, "wos_scopus.csv")
